...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
		...
		.authorizeHttpRequests(auth->auth.requestMatchers("/public/**").permitAll()
		        ...
		        .httpBasic(Customizer.withDefaults()); //only basic http no https
		return http.build();	  	
	}
	@Bean
	public UserDetailsService detailsService (PasswordEncoder passwordEncoder) {
		UserDetails user = User.withUsername("user").password(passwordEncoder.encode("adi")).roles("USER").build();
